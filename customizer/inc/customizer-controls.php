...
<<<<<<< HEAD
		...
		?>
			<i title='<?php echo $code; ?>' class='<?php echo "$class $class_fa_current"; ?> fa-control-icon-option'
			data-fa-code='<?php echo $class; ?>'></i>
		<?php
		}

		public function fontawesome_list($current_code, $input_id){
			$fa_codes = font_awesome_codes();
		?>
			<div class="fa-control" data-fa-current="<?php echo $current_code; ?>">
				<div class="current-fa">
					<i class="<?php echo $current_code; ?>"></i>
				</div>
				...
				<div class="collapsible-body font-awesome-list">
					...
					<div class="codes-list">
						...
						<div class="fa-normal-list">
							...
							<?php foreach($fa_codes['brands'] as $icon_code)
									$this->fontawesome_icon("fab", $icon_code,$current_code);
							?>
							</div>
							<!-- REGULAR -->
							<div class="collapsible-title fa-icons-group-title">Regular</div>
							<div class="collapsible-body fa-icons-group-body">
							<?php foreach($fa_codes['regular'] as $icon_code)
									$this->fontawesome_icon("far", $icon_code, $current_code);
							?>
							</div>
							<!-- SOLID -->
							<div class="collapsible-title fa-icons-group-title">Solid</div>
							<div class="collapsible-body fa-icons-group-body">
							<?php foreach($fa_codes['solid'] as $icon_code)
									$this->fontawesome_icon("fas", $icon_code ,$current_code);
							?>
							</div>
						</div>
					</div>
				</div>
				<input class="rb-sub-input" name="<?php echo $input_id; ?>" type="hidden" value="<?php echo $current_code; ?>">
			</div>
		<?php
		}
...
<<<<<<< HEAD
		...
		public function add_empty_inputs($inputs){
			$result = array();
			$inputs_types = $this->inputs_types;
			//Adds every input that doesnt have a value
			foreach( $inputs_types as $input_type_key => $input_info ){
				if( !isset($inputs[$input_type_key]) )
					$result[$input_type_key] = "";
				else
					$result[$input_type_key] = $inputs[$input_type_key];
			}
			return $result;
		}

		public function get_inputs_group_values($inputs_group){
			$inputs_types = $this->inputs_types;
			//Adds every input that doesnt have a value
			$final_group = array();
			foreach( $inputs_types as $input_type_key => $input_info ){
				$final_value = "";
				if( !isset($inputs_group[$input_type_key]) ){//There isnt a value saved for this input...
					if( isset($input_info['default']) )//Has default value?...
						$final_value = $input_info['default'];
				}
				else//There is a value saved...
					$final_value = $inputs_group[$input_type_key];//Assign it

				$final_group[$input_type_key] = $final_value;
			}
			return $final_group;
		}
		...
		public function get_all_inputs_values(){
			...
			if( is_array($inputs_groups) ){
				foreach($inputs_groups as $input_group_key => $inputs_values){
					//Adds every input that doesnt have a value
					$final_group = $this->get_inputs_group_values($inputs_values);
					$result[$input_group_key] = $final_group;
				}
			}
		...
		public function print_inputs_groups(){
			$inputs_groups = $this->get_all_inputs_values();
...
<<<<<<< HEAD
				...
				<div class="rb-tinymce-control">
					<div class="tinymce-placeholder">
						<textarea class="tinymce-content-preview"><?php echo $value; ?></textarea>
...
<<<<<<< HEAD
				...
				<?php elseif ( $type == "fontawesome" ): ?>
					<?php $this->fontawesome_list($value, $input_id); ?>
				<?php
...
<<<<<<< HEAD
			...
			if($this->collapsible_group){
				...
				if($this->collapsible_open)
					$title_classes .= " collapsible-open";
			}
			?>
			<label class="customize-control-multiple-inputs customize-control-inputs-generator
			<?php if(!$this->collapsible_group): ?> customizer-control-holder <?php endif; echo $this->label_classes; ?>"
			...
			data-dinamic-label-id="<?php echo $this->dinamic_label; ?>">
				<div class="<?php echo $title_classes; ?>">
					...
					<span class="customize-control-arrow top-control-arrow">
						<i class="fas fa-angle-down collapsible-arrow" aria-hidden="true"></i>
					</span>
					<?php endif;?>
=======
...
<<<<<<< HEAD
				...
				<?php endif;?>
					...
					<ul class="customizer-sortable-ul ui-sortable">
						<input type="text" class="rb-inputs-group-fake-input">
